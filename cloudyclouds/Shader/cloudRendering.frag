...
const vec3 ColorDark = vec3(0.34, 0.43, 0.51)*0.85;
const vec3 ColorLight = vec3(0.83, 0.83, 0.81)*1.15;
...
/*
	...
	*/
	...
	specularAmount = min(specularAmount, 1.0);

	// final color
	fragColor.a = alpha;
	float NDotL = max(0.0, dot(LightDirection_viewspace, normal_viewspace));
	float lightAmount = min(1.0, shadowing + (shadowing*0.2 + 0.2) * NDotL * fragColor.a);	// here also retouch spherical terms with alpha
